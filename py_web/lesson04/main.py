...
def handle_archive(filename: Path, target_folder: Path):
    ...
    try:
        ...
        logging.debug(f'Обман - это не архив {filename}!')
...
def handle_folder(folder: Path):
    try:
        ...
        logging.debug(f'Не удалось удалить папку {folder}')
...
def main(folder: Path):
    ...
    for file in parser.ZIP_ARCHIVES:
        handle_file(file, folder / 'archives')
...
def scan_folder(folder: Path):
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        futures = [executor.submit(parser.connect, folder)]
...
def get_value(*args):
    if len(args) < 1:
        logging.debug('Please enter a folder name')
    elif args[0]:
        parser.FOLDERS = []
